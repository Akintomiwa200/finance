...
export default function EditExpenseReportPage() {
  ...
  const { reports, loading, fetchReports, getReportById, updateReport } = useExpenseStore();
  ...
  useEffect(() => {
    if (initialized || reports.length === 0) return;
  ...
  }, [id, reports, getReportById, initialized]);
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(false);
    if (result) router.push(`/expenses/reports/${id}`);
  };
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
      <nav className="flex items-center gap-2 text-sm text-muted-foreground">
        ...
        <span>/</span>
        <span className="text-foreground">Edit Report</span>
      </nav>

      <div className="flex items-center gap-4">
        <Button variant="ghost" onClick={() => router.push(`/expenses/reports/${id}`)} className="gap-2">
          <ArrowLeft className="h-4 w-4" /> Back
        </Button>
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold tracking-tight">Edit Expense Report</h1>
          <p className="text-muted-foreground mt-1">Update expense report details and line items</p>
        </div>
      </div>
      ...
      <div className="flex justify-end gap-3">
        <Button variant="outline" onClick={() => router.push(`/expenses/reports/${id}`)} disabled={submitting}>Cancel</Button>
