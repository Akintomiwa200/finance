...
export default function ExpenseReportsPage() {
  ...
  const { reports, loading, fetchReports, deleteReport } = useExpenseStore();

  const [search, setSearch] = useState("");
  const [statusFilter, setStatusFilter] = useState("all");
  const [sortKey, setSortKey] = useState<"title" | "employeeName" | "totalAmount" | "status" | "submittedAt">("submittedAt");
  ...
  const stats = useMemo(() => {
    ...
    return { total, pending, approved, reimbursed, totalAmount };
  }, [reports]);
  ...
  const handleExport = () => {
    const headers = ["Title", "Employee", "Department", "Amount", "Status", "Submitted Date"];
    const rows = filtered.map((r) => [
      r.title, r.employeeName, r.department ?? "", r.totalAmount.toString(), r.status, r.submittedAt ?? "",
    ]);
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Create and manage employee expense reports</p>
        </div>
        <div className="flex gap-2">
          <Button variant="outline" onClick={handleExport} className="gap-2">
            <Download className="h-4 w-4" /> Export
          </Button>
          <Button onClick={() => router.push("/expenses/reports/new")} className="gap-2">
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-purple-600">{stats.reimbursed}</p>
              </div>
                  ...
                  <TableRow>
                    <TableCell colSpan={7} className="text-center py-12">
                      <div className="flex flex-col items-center gap-2">
                        ...
                        <p className="text-muted-foreground">Loading expense reports...</p>
                      </div>
                    </TableCell>
                  </TableRow>
                ) : paginated.length === 0 ? (
                    ...
                    <TableRow key={r.id}>
                      ...
                      <TableCell>
                        <div className="flex items-center gap-1">
                          <Button variant="ghost" size="sm" onClick={() => router.push(`/expenses/reports/${r.id}`)}>
                            <Eye className="h-4 w-4" />
                          </Button>
                          <Button variant="ghost" size="sm" onClick={() => router.push(`/expenses/reports/${r.id}/edit`)}>
