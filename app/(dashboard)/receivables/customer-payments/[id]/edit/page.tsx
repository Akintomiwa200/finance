...
export default function EditPaymentPage() {
  ...
  useEffect(() => {
    if (payment) {
      setForm({
        ...
        onlineReference: payment.onlineReference ?? "",
      });
    }
  }, [payment]);

  const filteredInvoices = useMemo(() => {
    if (!form.customerId) return invoices.filter((i) => i.balanceDue > 0 || i.id === form.invoiceId);
    return invoices.filter((i) => (i.customerId === form.customerId && i.balanceDue > 0) || i.id === form.invoiceId);
  }, [invoices, form.customerId, form.invoiceId]);
  ...
  const validate = () => {
    ...
    if (selectedInvoice && form.amount > selectedInvoice.balanceDue + (payment?.amount ?? 0)) {
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(false);
    if (result) router.push(`/receivables/customer-payments/${id}`);
  };
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
      <nav className="flex items-center gap-2 text-sm text-muted-foreground">
        ...
        <span>/</span>
        <button onClick={() => router.push(`/receivables/customer-payments/${id}`)} className="hover:text-foreground">{payment.paymentNumber}</button>
        <span>/</span>
        <span className="text-foreground">Edit</span>
      </nav>

      <div className="flex items-center gap-4">
        <Button variant="ghost" onClick={() => router.push(`/receivables/customer-payments/${id}`)} className="gap-2">
          <ArrowLeft className="h-4 w-4" /> Back
        </Button>
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold tracking-tight">Edit Payment</h1>
          <p className="text-muted-foreground mt-1">{payment.paymentNumber}</p>
        </div>
      </div>
      ...
      <div className="flex justify-end gap-3">
        <Button variant="outline" onClick={() => router.push(`/receivables/customer-payments/${id}`)} disabled={submitting}>Cancel</Button>
