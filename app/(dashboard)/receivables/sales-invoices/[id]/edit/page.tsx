...
export default function EditInvoicePage() {
  ...
  const { invoices, customers, loading, fetchInvoices, fetchCustomers, getInvoiceById, updateInvoice } = useReceivableStore();
  ...
  useEffect(() => {
    if (invoice && !initialized) {
      ...
      let k = 1;
      const mapped = invoice.lines.map((l) => ({
        key: k++,
      ...
      setLineItems(mapped.length ? mapped : [emptyLine(1)]);
      setNextKey(k);
      setInitialized(true);
    }
  }, [invoice, initialized]);
  ...
  const handleSubmit = async () => {
    ...
    const result = await updateInvoice(id, {
      ...
      taxAmount,
      totalAmount,
      balanceDue: totalAmount - (invoice?.amountPaid ?? 0),
      lines: lineItems
    ...
    setSubmitting(false);
    if (result) router.push(`/receivables/sales-invoices/${id}`);
  };
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
      <nav className="flex items-center gap-2 text-sm text-muted-foreground">
        ...
        <span>/</span>
        <button onClick={() => router.push(`/receivables/sales-invoices/${id}`)} className="hover:text-foreground">{invoice.invoiceNumber}</button>
        <span>/</span>
        <span className="text-foreground">Edit</span>
      </nav>

      <div className="flex items-center gap-4">
        <Button variant="ghost" onClick={() => router.push(`/receivables/sales-invoices/${id}`)} className="gap-2">
          <ArrowLeft className="h-4 w-4" /> Back
        </Button>
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold tracking-tight">Edit Invoice</h1>
          <p className="text-muted-foreground mt-1">Update {invoice.invoiceNumber}</p>
        </div>
      </div>
      ...
      <div className="flex justify-end gap-3">
        <Button variant="outline" onClick={() => router.push(`/receivables/sales-invoices/${id}`)} disabled={submitting}>Cancel</Button>
