...
export default function EditPurchaseOrderPage() {
  ...
  const { vendors, fetchVendors, getPOById, fetchPurchaseOrders, updatePO, loading } = usePayableStore();
  ...
  useEffect(() => {
    ...
    if (po.status !== "draft") {
      router.push(`/payables/purchase-orders/${id}`);
      return;
    }
    ...
    setDeliveryMethod(po.deliveryMethod);
    setDeliveryAddress(po.deliveryAddress || "");
    setDeliveryNotes(po.deliveryNotes || "");
    setNotes(po.notes || "");
    setTaxRate(po.taxRate ?? 7.5);
    setLines(po.lines.map((l) => ({ ...l })));
    setInitialized(true);
  }, [id, getPOById, fetchPurchaseOrders, loading, initialized, router]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const result = await updatePO(id, {
      vendorId,
    ...
    setSubmitting(false);
    if (result) router.push(`/payables/purchase-orders/${id}`);
  };
    ...
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <Card>
          ...
          <CardContent>
            <Textarea value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Additional notes or comments..." rows={3} />
          </CardContent>
        </Card>
        ...
        <div className="flex justify-end gap-3">
          ...
          <Button type="submit" disabled={submitting || !vendorId || !expectedDeliveryDate || lines.length === 0} className="gap-2">
            ...
            {submitting ? "Saving..." : "Update Purchase Order"}
          </Button>
        </div>
      </form>
    </div>
